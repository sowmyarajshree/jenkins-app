...
def get_hours(filters):
	...
	return frappe.db.sql("""select ode.item_code as item_code,ode.item_name as item_name,
						...
						case when ode.planned_shots >= 0 then 'PR' else 0 end as shots

					from
					    `tabOperation Details Entry` ode
					where
					    ode.operation = "PDC" and
					...
					select
						...
						case when ode.actual_shots >= 0 then 'P' else 0 end as shots
					...
					select
						...
						case when ode.rejected_shots >= 0 then 'A' else 0 end as shots
