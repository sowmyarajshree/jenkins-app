...
def get_data(filters):
	...
	return frappe.db.sql("""
		SELECT
			sum(std.qty) as overall_plan,sum(std.qty)/3 as per_shift_plan,st.posting_date as date,st.stock_entry_type,
			st.from_warehouse , std.parent as std_parent, std.qty,std.t_warehouse as target_warehouse,std.item_code, std.item_name,
		...
		FROM
			...
			`tabStock Entry` st
		WHERE
			st.docstatus = 1 and st.stock_entry_type in ("Material Transfer","Full Rejection") and
			std.item_group in ("Finished Goods","Semi Finished Goods")
			and st.from_warehouse = "Incoming Store - ECE" and
...
def get_conditions(filters):
	...
	if filters.get("warehouse"):
		conditions +=" and st.from_warehouse = %(warehouse)s"
