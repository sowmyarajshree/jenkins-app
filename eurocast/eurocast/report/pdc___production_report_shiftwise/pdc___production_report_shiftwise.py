...
def get_hours(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		SELECT
			op.name , op.posting_date, op.workstation,op.shift_type,op.operator_name,od.item_code, sum(od.actual_shots) as completed_qty, sum(od.rejected_shots) as rejected_qty, sum(od.planned_shots) as planned_qty,
			format(sum(od.hour_output)/count(od.idx),2) as hour_output, op.operation, format((sum(od.actual_shots)/sum(od.planned_shots) * 100),2) as efficiency, format((sum(od.rejected_shots) /sum(od.actual_shots) * 100),2) as rejection,
		...
		WHERE
			od.parent = op.name and op.docstatus = 1 and op.operation = "PDC" %s
		GROUP BY
