...
def get_data(filters):
	...
	FROM
	    `tabLabour Attendance` la, `tabLabour Detail` lad

    WHERE
        lad.parent = la.name and la.docstatus = 1 %s  """ % conditions, filters, as_dict=1)
...
def get_conditions(filters):
	...
	if filters.get("posting_date"):
		conditions +=" and la.posting_date = %(posting_date)s"
	return conditions
